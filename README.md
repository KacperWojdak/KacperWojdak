<div align="center">

# Hej, jestem Kacper 👋 / Hi, I'm Kacper 👋

### Frontend Developer 💻 · Backend 🤏 · Piłkarz ⚽ · Wieczny debugger nocnych commitów 🌙

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&pause=1000&color=2F80ED&center=true&vCenter=true&width=600&lines=Next.js+%2B+TypeScript+%2B+Tailwind;Buduje+strony+dla+lokalnej+spo%C5%82eczno%C5%9Bci;Frontend+by+day%2C+bugs+by+night;" alt="Typing SVG" />

</div>

---

## 🇵🇱 O mnie

🎓 Ukończyłem studia informatyczne na kierunku "Programowanie aplikacji mobilnych i internetowych", programowaniem zajmuję się od technikum (2017) — zaczynałem jak każdy od czystego HTML/CSS/JS.

⚽ Aktualnie jestem bramkarzem w **GKS Legsad Kościelec** — i sam zbudowałem oficjalną stronę klubu (Next.js 15 + Sanity CMS), bo skoro już tam gram, to w taki sposób najlepiej wykorzystam swoje umiejętności.

🎮 W wolnych chwilach lubię grać w gry multiplayer od gier sportowych po fps'y lecz również uwielbiam gry single-player.

🕹️ Wcześniej programowałem i tworzyłem też w Unity (2D/3D/VR) — VR Escape Room był moją pracą inżynierską.

🛠️ Lubię robić rzeczy metodycznie: plik po pliku, komponent po komponencie, z porządnym review na każdym etapie.

---

## 🇬🇧 About Me

🎓 I graduated with a degree in computer science, specializing in “Mobile and Web Application Programming,” and I've been programming since technical high school (2017)—I started out like everyone else with plain HTML/CSS/JS.

⚽ I’m currently the goalkeeper for **GKS Legsad Kościelec**— and I built the club’s official website myself (Next.js 15 + Sanity CMS), because since I play there, this is the best way for me to put my skills to use.

🎮 In my free time, I like to play multiplayer games, ranging from sports games to FPSs, but I also love single-player games.

🕹️ Previously, I also programmed and developed in Unity (2D/3D/VR) — VR Escape Room was my engineering thesis project.

🛠️ I like to work methodically: file by file, component by component, with a thorough review at every stage.

---

## 🧰 Tech Stack

<div align="center">

![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)

![Sanity](https://img.shields.io/badge/Sanity-F03E2F?style=for-the-badge&logo=sanity&logoColor=white)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-0055FF?style=for-the-badge&logo=framer&logoColor=white)
![Zustand](https://img.shields.io/badge/Zustand-433E38?style=for-the-badge)
![Netlify](https://img.shields.io/badge/Netlify-00C7B7?style=for-the-badge&logo=netlify&logoColor=white)

![Unity](https://img.shields.io/badge/Unity-100000?style=for-the-badge&logo=unity&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=csharp&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

</div>

🤏 Backend? Znam, ale w takiej ilości, żeby wiedzieć, że wolę zostać na froncie. Tworzyłem proste zapytania i połączenia front + back (Sanity, API routes, trochę Node), ale serce zdecydowanie zostaje we frontendzie.

---

## 🚀 Wyróżnione projekty / Featured Projects

### ⚽ [legsadkoscielec_website](https://github.com/KacperWojdak/legsadkoscielec_website)
Oficjalna strona klubu piłkarskiego GKS Legsad Kościelec — Next.js 15 + Sanity CMS, statystyki sezonu liczone automatycznie z raportów meczowych, on-demand revalidation, dynamiczne OG images.
> Official website of GKS Legsad Kościelec football club — Next.js 15 + Sanity CMS, live-calculated season stats from match reports, on-demand revalidation, dynamic OG images.

### 🚗 [mvp-detailing-website](https://github.com/KacperWojdak/mvp-detailing-website)
(Wciąż w produkcji) Nowoczesna strona dla firmy detailingowej — Vite + React + TypeScript, Tailwind v4, Zustand, tryb ciemny/jasny, pełna responsywność (nawet na ultrawide'ach).
> (Still in production) Modern website for a car detailing business — Vite + React + TypeScript, Tailwind v4, Zustand, dark/light mode, fully responsive (even on ultrawide screens).

### 🕶️ [VR_EscapeRoom](https://github.com/KacperWojdak/VR_EscapeRoom)
Projekt VR stworzony na pracę inżynierską — Unity + C#.
> VR project built for my engineering thesis — Unity + C#.

### 📸 [asiam_photography](https://github.com/KacperWojdak/asiam_photography)
(Wciąż w produkcji) Portfolio fotograficzne oraz reklama (portrety, eventy, sport) — Next.js + Tailwind CSS v4.
> (Still in production) Photography portfolio and advertisement (portraits, events, sports) — Next.js + Tailwind CSS v4.

---

## 📊 GitHub Stats

<div align="center">
  
<img src="https://github-stats-extended.vercel.app/api?username=KacperWojdak&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="GitHub Stats" height="165"/>
<img src="https://github-stats-extended.vercel.app/api/top-langs/?username=KacperWojdak&layout=compact&theme=tokyonight&hide_border=true" alt="Top Languages" height="165"/>

</div>

---
## 📫 Skontaktuj się / Get in touch

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kacper-wojdak/)
[![Gmail](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kacperwojdak.dev@gmail.com)

*Legnica, Poland 🇵🇱*

</div>
